...
export async function GET(req: Request) {
  ...
  const ingestRes = await fetch(`${origin}/api/jobs/news/ingest`, { cache: 'no-store' }).catch(() => null);
  const ingest = ingestRes && ingestRes.ok ? await ingestRes.json() : { ok: false };
  const enrichRes = await fetch(`${origin}/api/jobs/news/enrich`, { cache: 'no-store' }).catch(() => null);
