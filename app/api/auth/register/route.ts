...
import { z } from 'zod';
import { ensureUsersTable, findUserByEmail, insertUser, hashPassword } from '@/lib/db';
...
export async function POST(req: NextRequest) {
  try {
    ...
    await insertUser({ id, email: data.email, password_hash, name: data.name ?? null });
    ...
    // form → redirect do /konto; JSON → {ok:true}
    const ct = req.headers.get('content-type') || '';
    if (ct.includes('application/json')) {
      return NextResponse.json({ ok: true, userId: id, email: data.email });
    }
    return NextResponse.redirect(new URL('/konto', req.url), { status: 302 });
  } catch (err: any) {
