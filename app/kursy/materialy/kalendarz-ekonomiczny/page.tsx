...
import { KALENDARZ_EKONOMICZNY_LEKCJE } from "./lessons";

export const metadata: Metadata = {
  title: "Kalendarz ekonomiczny — spis lekcji | Materiały",
  description:
    "Czytanie publikacji makro, zmienność wokół danych i przygotowanie scenariuszy pod FX/CFD.",
};

export default function Page() {
  const lessons = KALENDARZ_EKONOMICZNY_LEKCJE.map(({ slug, title, blurb, minutes }) => ({
    slug,
    title,
    blurb,
    minutes,
      ...
      <section className="relative mt-6 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#0b1220] via-[#0a0f1a] to-[#080d16] p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.04)_inset] md:p-8 md:shadow-lg">
        ...
        <header className="relative">
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white md:text-4xl">
            Kalendarz ekonomiczny — spis lekcji
          </h1>
          <p className="mt-3 max-w-2xl text-pretty text-sm leading-relaxed text-slate-300 md:text-base">
            Nauczysz się czytać publikacje makro, rozumieć kiedy rynek naprawdę przyspiesza i kiedy lepiej odpuścić handel
            zamiast pakować się w chaos.
          </p>
        </header>

        <MaterialyModulModuleProgress courseId={KALENDARZ_EKONOMICZNY_COURSE_ID} lessonSlugs={lessonSlugs} />
      </section>

      <MaterialyModulModuleLessonGrid
        courseId={KALENDARZ_EKONOMICZNY_COURSE_ID}
        moduleBasePath="/kursy/materialy/kalendarz-ekonomiczny"
        lessons={lessons}
      />
