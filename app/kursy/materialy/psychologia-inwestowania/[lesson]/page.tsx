...
import {
  PSYCHOLOGIA_LEKCJA_SEKCJE,
  PSYCHOLOGIA_LEKCJE,
  getPsychologiaLesson,
  psychologiaLessonIndex,
} from "../lessons";

type PageProps = { params: Promise<{ lesson: string }> };

const MODULE_PATH = "/kursy/materialy/psychologia-inwestowania";
const MODULE_BREADCRUMB = "Psychologia inwestowania";

export function generateStaticParams() {
  return PSYCHOLOGIA_LEKCJE.map((l) => ({ lesson: l.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { lesson: slug } = await params;
  const lekcja = getPsychologiaLesson(slug);
  if (!lekcja) return { title: "Lekcja | Psychologia inwestowania" };
  return {
    title: `${lekcja.title} | Psychologia inwestowania`,
...
export default async function Page({ params }: PageProps) {
  ...
  const idx = psychologiaLessonIndex(slug);
  const lessonNumber = idx + 1;
  const total = PSYCHOLOGIA_LEKCJE.length;
  const lessonSlugs = PSYCHOLOGIA_LEKCJE.map((l) => l.slug);
  const prev = idx > 0 ? PSYCHOLOGIA_LEKCJE[idx - 1] : undefined;
  const next = idx < PSYCHOLOGIA_LEKCJE.length - 1 ? PSYCHOLOGIA_LEKCJE[idx + 1] : undefined;
    ...
    <main className="min-h-[70vh] text-white">
      <div className="mx-auto max-w-3xl px-4 pb-20 pt-6 sm:px-6 sm:pt-8 md:pt-10">
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
          ...
          >
            ← Wróć do psychologii inwestowania
          </Link>
        </div>
          ...
          <div className="relative flex flex-col gap-4">
            ...
            <p className="mt-1 max-w-2xl border-t border-white/10 pt-4 text-xs leading-relaxed text-slate-500">
              {next ? (
                ...
                <>
                  Ostatnia lekcja modułu — wróć do spisu albo zrób jeden konkretny wpis w dzienniku przed kolejną sesją.
                </>
              )}
            </p>
          </div>
        ...
        <div className="mt-10 flex flex-col gap-10 md:mt-12 md:gap-12 lg:gap-14">
          {PSYCHOLOGIA_LEKCJA_SEKCJE.map(({ pole, tytul, variant, eyebrow }) => (
