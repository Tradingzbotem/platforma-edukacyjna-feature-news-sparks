...
export function generateStaticParams() {
  return FORMACJE_LEKCJE.map((l) => ({ lesson: l.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { lesson: slug } = await params;
  const lekcja = getFormacjeLesson(slug);
  if (!lekcja) return { title: "Lekcja | Formacje świecowe" };
  return {
    title: `${lekcja.title} | Formacje świecowe`,
...
export default async function Page({ params }: PageProps) {
  ...
  const idx = formacjeLessonIndex(slug);
  const lessonNumber = idx + 1;
  const total = FORMACJE_LEKCJE.length;
  const lessonSlugs = FORMACJE_LEKCJE.map((l) => l.slug);
  const prev = idx > 0 ? FORMACJE_LEKCJE[idx - 1] : undefined;
  const next = idx < FORMACJE_LEKCJE.length - 1 ? FORMACJE_LEKCJE[idx + 1] : undefined;
    ...
    <main className="min-h-[70vh] text-white">
      <div className="mx-auto max-w-3xl px-4 pb-20 pt-6 sm:px-6 sm:pt-8 md:pt-10">
        ...
        <div className="mt-10 flex flex-col gap-10 md:mt-12 md:gap-12 lg:gap-14">
          {FORMACJE_LEKCJA_SEKCJE.map(({ pole, tytul, variant, eyebrow }) => (
        ...
        >
          {prev ? (
            <Link
              href={`/kursy/materialy/formacje-swiecowe/${prev.slug}`}
          ...
          {next ? (
            <Link
              href={`/kursy/materialy/formacje-swiecowe/${next.slug}`}
