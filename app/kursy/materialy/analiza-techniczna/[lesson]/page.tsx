...
import { ANALIZA_LEKCJA_SEKCJE, ANALIZA_LEKCJE, analizaLessonIndex, getAnalizaLesson } from "../lessons";
type PageProps = { params: Promise<{ lesson: string }> };

export function generateStaticParams() {
  return ANALIZA_LEKCJE.map((l) => ({ lesson: l.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { lesson: slug } = await params;
  const lekcja = getAnalizaLesson(slug);
  if (!lekcja) return { title: "Lekcja | Analiza techniczna" };
  return {
    title: `${lekcja.title} | Analiza techniczna`,
...
export default async function Page({ params }: PageProps) {
  ...
  const idx = analizaLessonIndex(slug);
  const lessonNumber = idx + 1;
  const total = ANALIZA_LEKCJE.length;
  const lessonSlugs = ANALIZA_LEKCJE.map((l) => l.slug);
  const prev = idx > 0 ? ANALIZA_LEKCJE[idx - 1] : undefined;
  const next = idx < ANALIZA_LEKCJE.length - 1 ? ANALIZA_LEKCJE[idx + 1] : undefined;
    ...
    <main className="min-h-[70vh] text-white">
      <div className="mx-auto max-w-3xl px-4 pb-20 pt-6 sm:px-6 sm:pt-8 md:pt-10">
          ...
          <div className="relative flex flex-col gap-4">
            ...
            <p className="mt-1 max-w-2xl border-t border-white/10 pt-4 text-xs leading-relaxed text-slate-500">
              {next ? (
                ...
                <>Ostatnia lekcja modułu — wróć do spisu lub utrwal to na wykresie w praktyce.</>
              )}
            </p>
          </div>
        </header>
        ...
        <div className="mt-10 flex flex-col gap-10 md:mt-12 md:gap-12 lg:gap-14">
          {ANALIZA_LEKCJA_SEKCJE.map(({ pole, tytul, variant, eyebrow }) => (
        ...
        >
          {prev ? (
            <Link
              href={`/kursy/materialy/analiza-techniczna/${prev.slug}`}
          ...
          {next ? (
            <Link
              href={`/kursy/materialy/analiza-techniczna/${next.slug}`}
